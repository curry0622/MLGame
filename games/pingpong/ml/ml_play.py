...
PLATFORM_WIDTH = 40
PLATFORM_HEIGHT = 30
PLATFORM_HALF_WIDTH = PLATFORM_WIDTH / 2

BLOCKER_WIDTH = 30
BLOCKER_HEIGHT = 20
BLOCKER_HALF_WIDTH = BLOCKER_WIDTH / 2
BLOCKER_HALF_HEIGHT = BLOCKER_HEIGHT / 2
...
class MLPlay:
    def __init__(self, side):
        ...
        # import models
        with open(os.path.join(os.path.dirname(__file__), 'model_1P_win_good.pickle'), 'rb') as f:
            self.model_1P = pickle.load(f)
        with open(os.path.join(os.path.dirname(__file__), 'model_2P_win_good.pickle'), 'rb') as f:
    ...
    def update(self, scene_info):
        ...
        if not self.ball_served:
            ...
            self.predict()
            platform_x = self.scene_info["platform_1P"][0] if self.side == "1P" else self.scene_info["platform_2P"][0]
            x = np.array([
                platform_x,
                self.predict_ball_x
            ]).reshape((1, -1))
